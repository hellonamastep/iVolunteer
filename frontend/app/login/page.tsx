    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-b from-[#E9FDF1] to-[#E9FDF1]/80 font-['Manrope'] px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="bg-[#FFFDF9] w-full max-w-xs sm:max-w-sm md:max-w-md rounded-2xl shadow-lg border border-[#f0f0f0] overflow-hidden">
        ...
        <div className="flex border-b border-gray-100">
          <div className="w-1/2 text-center py-3 font-semibold text-[#3ABBA5] border-b-2 border-[#3ABBA5] text-sm sm:text-base">Login</div>
          <Link href="/signup" className="w-1/2 text-center py-3 font-semibold text-gray-400 hover:text-[#3ABBA5] transition-all text-sm sm:text-base">Sign Up</Link>
        </div>

        <form
          onSubmit={(e) => { e.preventDefault(); e.stopPropagation(); handleSubmit(onSubmit)(e); }}
          className="px-8 pt-8 pb-10 text-gray-700"
        >
          <h2 className="text-lg sm:text-xl md:text-2xl font-semibold mb-1 text-gray-800 text-center sm:text-left">Welcome Back!</h2>
          <p className="text-xs sm:text-sm text-gray-500 mb-4 sm:mb-6 text-center sm:text-left">Continue your journey to make an impact</p>
          ...
          {linkedBanner && (
            <div className="bg-yellow-100 border border-yellow-300 text-yellow-800 px-4 py-3 rounded mb-4 text-xs sm:text-sm">
              Your account was linked to Google. You can now sign in with Google.
            </div>
          )}
          ...
          <div className="mb-4 sm:mb-5">
            <div className="flex justify-between items-center mb-2">
              ...
              <Link href="/forgot-password" className="text-xs sm:text-sm text-[#3ABBA5] hover:underline">Forgot password?</Link>
            </div>
            <div className="relative">
        ...
        <div className="text-center pb-4 sm:pb-6 px-4">
          <p className="text-xs sm:text-sm text-gray-500">
            Don't have an account? <Link href="/signup" className="text-[#3ABBA5] font-semibold hover:underline">Sign Up</Link>
          </p>
        </div>
      </div>
    </div>
