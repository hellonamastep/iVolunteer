...
export const AuthProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  // Signup keeps existing behavior (immediate session after register)
  const signup = async (signupData: SignupData): Promise<boolean> => {
    setIsLoading(true);
    try {
      const res = await api.post("/v1/auth/register", signupData, { withCredentials: true });
      const data = unwrap<VerifyAuthResponse>(res);
      ...
      const mappedUser: User = {
        ...
        cloudinaryPublicId: data.user.cloudinaryPublicId,
      };
  ...
  // Step 1: request OTP only. No user/tokens here.
  const login = async (email: string, password: string, role: UserRole = "user"): Promise<boolean> => {
    ...
    try {
      ...
      const body = unwrap<{ otpRequired?: boolean }>(res);
      const otpRequired = body?.otpRequired ?? true;

      if (otpRequired) {
        toast({ title: "OTP Sent", description: "Check your email for the 6-digit code." });
      }
      return Boolean(otpRequired);
    } catch (err: any) {
      console.error("Login failed:", err.response?.data?.message || err.message);
      toast({
        title: "Login Failed",
  ...
  // Step 2: verify OTP then persist session.
  const verifyOtp = async (email: string, otp: string): Promise<boolean> => {
    setIsLoading(true);
    try {
      const res = await api.post(
