...
const API_BASE_URL = `${process.env.NEXT_PUBLIC_API_BASE_URL || 'http://13.127.114.186:5000/api'}/v1`;
...
export const ParticipationRequestProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const createParticipationRequest = async (eventId: string, message?: string): Promise<boolean> => {
    if (!user) {
      toast({
        title: "Error",
        description: "You must be logged in to request participation",
        variant: "destructive",
      });
      return false;
    }
  ...
  const acceptRequest = async (requestId: string): Promise<boolean> => {
    if (!user || user.role !== "ngo") {
      toast({
        title: "Error",
        description: "Only event creators can accept requests",
        variant: "destructive",
      });
      return false;
    }
  ...
  const rejectRequest = async (requestId: string, rejectionReason?: string): Promise<boolean> => {
    if (!user || user.role !== "ngo") {
      toast({
        title: "Error",
        description: "Only event creators can reject requests",
        variant: "destructive",
      });
      return false;
    }
  ...
  const fetchUserRequests = async (status?: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Don't log error for invalid token - it's expected during logout
  ...
  useEffect(() => {
    if (user) {
      if (user.role === "ngo") {
        fetchIncomingRequests().catch((err) => {
          console.error("Failed to initialize incoming requests:", err);
        });
        fetchStats().catch((err) => {
          console.error("Failed to initialize stats:", err);
        });
      } else {
        fetchUserRequests().catch((err) => {
          console.error("Failed to initialize user requests:", err);
        });
      }
    } else {
