...
import { User } from "../models/User.js";
import { ApiError } from "../utils/ApiError.js";
import { sendEmail } from "../utils/email.js"; // Resend-based email util
...
const sendOtp = async (email) => {
  ...
  await sendEmail({
    ...
    html: `
      <div style="font-family: Arial, sans-serif; padding: 20px; background: #f5f5f5;">
        <div style="max-width: 480px; margin: auto; background: #fff; border-radius: 12px; padding: 24px; box-shadow: 0 6px 20px rgba(0,0,0,0.08);">
          <h2 style="text-align:center; color:#14b674; margin-bottom:8px;">MAILSTEP Secure Access</h2>
          <p style="text-align:center; color:#555; font-size:15px;">Your One-Time Login Code</p>
          <div style="margin-top:30px; text-align:center;">
            <div style="font-size:32px; letter-spacing:8px; font-weight:700; color:#000;">${otp}</div>
            <p style="margin-top:16px; color:#666; font-size:14px;">
              This code expires in <strong>5 minutes</strong>.<br/>
              Do not share it with anyone.
            </p>
          ...
          <hr style="margin:32px 0; border:none; border-top:1px solid #eee;" />
          <p style="text-align:center; font-size:12px; color:#888;">Powered securely by <strong>MAILSTEP</strong>.</p>
        </div>
      </div>
    `
  });
